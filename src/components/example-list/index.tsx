...
const ExampleList: React.FC = () => {
  ...
  React.useEffect(() => {
    ...
    if (tags) {
      ...
      const validTags = tags.split(",").filter((tag) => tags.includes(tag));
    ...
    return (
      ...
      >
        ...
        >
          <a
            href={example.source}
            className={clsx("flex items-center gap-2", "no-underline")}
            target="_blank"
            rel="noreferrer"
          >
            <GithubIconOutlined className={clsx("w-4 h-4 2xl:w-6 2xl:h-6")} />
            <span>Code</span>
          </a>
          ...
          return (
            <button
              ...
              className={clsx(
                ...
                "cursor-pointer",
                {
                  "text-gray-600 dark:text-gray-400": !isActive || allSelected,
                  "text-gray-900 dark:text-gray-0": isActive && !allSelected,
                  "bg-gray-100 dark:bg-gray-800": !isActive || allSelected,
                },
              )}
              style={{
                borderWidth: "1px",
                borderColor:
                  isActive && !allSelected ? `${tagColor}40` : "transparent",
                backgroundColor:
                  isActive && !allSelected ? `${tagColor}33` : "",
              }}
              onClick={() => updateFilters(name)}
            >
              <div
                className={clsx("w-2 h-2 rounded-full", {
                  "bg-gray-400 dark:bg-gray-600": !isActive || allSelected,
                })}
                style={{
                  backgroundColor: isActive && !allSelected ? tagColor : "",
                }}
              />
              {getTagName(name)}
            </button>
          ...
          >
            <Disclosure.Button
              className={clsx(
                ...
                {
                  "border-b border-gray-100 dark:border-gray-700": open,
                },
              )}
            >
              <TriangleDownIcon
    ...
    return (
      <div className="flex flex-col">
        ...
        >
          ...
          <span
            className={clsx("hidden md:block", "text-base", "text-gray-500")}
          >
            {filteredExamples.length} examples
          </span>
        </div>
        <div className="hidden md:flex">{renderFilterList()}</div>
        <div className="block md:hidden">{renderDisclosureFilterList()}</div>
      </div>
    );
  };
